...
# Combining the final pipeline with batch processing and result saving
def run_pipeline_on_all_trials():
    PAIR_LIST = ["1&1", "1&2", "1&3", "1&4", "2&2", "2&3", "2&4", "3&3", "3&4", "4&4"]
    TRIALS = ["t1", "t2", "t3"]
    BASE_PATH = "filtered"
    SOUND_SPEED = 162900
    REF_IDX = 0
    ...
    for pair in PAIR_LIST:
        for trial in TRIALS:
            file_path = f"{BASE_PATH}/filtered_piezo_voltage_log-{pair}-{trial}.csv"
            ...
            try:
                ...
                for idx in peak_indices:
                    record = []
                    peak_records.append((time[idx], REF_IDX, idx, ref_signal[idx]))
                    for sid in range(signals.shape[1]):
                        if sid == REF_IDX:
                            record.append(0.0)
                            continue
                ...
                for label in [0, 1]:
                    ...
                    peak_indices, _ = find_peaks(smooth_envelope, height=0.6 * np.max(smooth_envelope))
                ...
                for cid in [0, 1]:
                    ...
                    if location is None:
                        location, _, _, _ = grid_search_localization(sensor_coords, tdoas, ref_index=REF_IDX, sound_speed=SOUND_SPEED)

                    results.append({
                        "Pair": pair,
                        "Trial": trial,
                        "Speaker": "A" if cid == 0 else "B",
                        "Est. X (cm)": round(location[0], 2),
                        "Est. Y (cm)": round(location[1], 2)
                    })
    ...
    results_df.to_csv("All_Final_Estimated_Locations.csv", index=False)
    return results_df

# Run the pipeline and return the DataFrame
results_df = run_pipeline_on_all_trials()
